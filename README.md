...
To update the database use this command from the Web project folder (replace `BasketballStats` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../BasketballStats.Infrastructure/BasketballStats.Infrastructure.csproj -s BasketballStats.Web.csproj
```
